 ...
 *
 * This program is for the DS90UB9702 FPD Link IV deserializer in connection
...
//                           (val << TI9702_CSI_TX_SPEED) | (val2 << TI9702_CSI_TX_SPEED2)); //NEW
	...
	for (; ser_nr < priv->num_ser; ser_nr++) {
		...
		for(i = 0; i < 50; i++) {
			msleep(10);
			err = ds90ub9702_read(priv, TI9702_REG_DEVICE_STS, &val);
			if(unlikely(err))
				goto ser_init_failed;
...
MODULE_DESCRIPTION("i2c ds90ub9702 driver");
